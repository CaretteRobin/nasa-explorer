<template>
  <section>
    <div class="mb-6 flex flex-wrap items-end gap-3 justify-between">
      <div>
        <h1 class="font-display text-3xl">Météo spatiale (DONKI)</h1>
        <p class="text-slate-300">Éruptions solaires (FLR), CME et tempêtes géomagnétiques (GST)</p>
      </div>
      <div class="flex items-center gap-2">
        <select v-model="type" class="bg-white/10 border border-white/10 rounded px-3 py-2 text-slate-100">
          <option value="FLR">Éruptions (FLR)</option>
          <option value="CME">CME</option>
          <option value="GST">Géomagnétique (GST)</option>
        </select>
    ...
    <div v-else class="space-y-3">
      <div v-for="e in items" :key="e.activityID || e.flrID || e.gstID" class="rounded-2xl glass p-4">
        ...
        <div class="text-slate-100 font-medium">{{ e.classType || e.note || e.cmeAnalyses?.[0]?.type || 'Événement' }}</div>
